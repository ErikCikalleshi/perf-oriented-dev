...
void insert(int block_index, int elem_index, void *value, size_t eleme_size);
...
void delete(int block_index, int elem_index, size_t eleme_size)
{
    ...
    // make the value 0
    int zero = 0;
    memcpy((char *)current_block->head->value + elem_index * eleme_size, &zero, eleme_size);
}
...
void print_linked_list()
{
    ...
    while (current_block != NULL)
    {
        ...
        print_block_as_linked_list(block_index, current_block->max_elem_per_chunk);
        printf("\n");
        current_block = current_block->next_block;
        block_index++;
    }
...
void run_example()
{
    ...
    int insert_value = 99;
    insert(0, 0, &insert_value, elem_size);
